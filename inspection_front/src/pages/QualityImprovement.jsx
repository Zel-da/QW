...
function QualityKpiSection({ items, onKpiClick }) {
    const kpiData = useMemo(() => {
        return items.reduce((acc, item) => {
            const calculatedStatus = item.calculated_status; // Use calculated status from backend
...
function QualityListSection({ user, items, onRowClick, onAddSuccess }) {
    ...
    useEffect(() => {
        ...
        const calculatedStatuses = [...new Set(items.map(item => item.calculated_status))];
        setFilterOptions({ 
    ...
    const filteredItems = useMemo(() => {
        return items.filter(item => {
            ...
            const itemCalculatedStatus = item.calculated_status; // Calculate status for each item
            return (
...
function QualityImprovement({ user }) {
    ...
    const filteredByKpi = useMemo(() => {
        ...
        return allItems.filter(item => item.calculated_status === statusFilter);
